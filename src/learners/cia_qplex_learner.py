...
from modules.mixers.qplex import DMAQer, DMAQ_QattenMixer
import torch as th
...
class CIAQPlexLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.identity = nn.Linear(self.args.episode_limit, self.args.n_agents)
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        if mixer is not None:
            individual_qvals = chosen_action_qvals
            if self.args.mixer == "dmaq_qatten":
                ans_chosen, q_attend_regs, head_entropies = \
                    mixer(individual_qvals, batch["state"][:, :-1], is_v=True)
                ans_adv, _, _ = mixer(individual_qvals, batch["state"][:, :-1], actions=actions_onehot,
                                      max_q_i=max_action_qvals, is_v=False)
                chosen_action_qvals = ans_chosen + ans_adv
            else:
                ans_chosen = mixer(individual_qvals, batch["state"][:, :-1], is_v=True)
                ans_adv = mixer(individual_qvals, batch["state"][:, :-1], actions=actions_onehot,
                                max_q_i=max_action_qvals, is_v=False)
                chosen_action_qvals = ans_chosen + ans_adv

            # ---------------------------  CIA Start  ---------------------------
            temporal_credits = th.autograd.grad(chosen_action_qvals.sum(), individual_qvals, create_graph=True, retain_graph=True)[0]
            ...
            similarity = self.identity(temporal_credits)
            similarity = th.transpose(similarity, dim0=1, dim1=2)
            similarity = similarity.reshape(-1, self.args.n_agents)

            label = th.arange(0, self.args.n_agents)
            label = label.repeat(self.args.batch_size).to(batch.device)

            cl_loss = self.loss(similarity, label)
            # ---------------------------  CIA End  ---------------------------
        ...
        if self.args.mixer == "dmaq_qatten":
            td_loss = (masked_td_error ** 2).sum() / mask.sum() + q_attend_regs
        else:
            td_loss = (masked_td_error ** 2).sum() / mask.sum()

        loss = td_loss + self.args.alpha * cl_loss
